...
namespace TGC.Group.Model.Parcelas
{
    class PitH : Parcela
    {
        public PitH(TGCVector3 Position, string grassTexture, string wallTexture, string columnTexture, string topTexture)
        {
            this.Position = Position;

            //Se define el terrno de la parcela
            var floor = new TgcPlane(new TGCVector3(Position.X, Position.Y - 20, Position.Z), new TGCVector3(50, 0, 50), TgcPlane.Orientations.XZplane, TgcTexture.createTexture(grassTexture), 4, 4).toMesh("floor");
            meshes.Add(floor);
